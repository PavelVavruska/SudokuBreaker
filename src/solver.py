...
class Solver:
    ...
    def free_at_position(new_sudoku, y, x):
        ...
        first_step_avail = init_avail.intersection(
            Detector.free_in_column(new_sudoku, x))

        second_step_avail = first_step_avail.intersection(
            Detector.free_in_row(new_sudoku, y))

        third_step_avail = second_step_avail.intersection(
            Detector.free_in_squere3x3(new_sudoku, y, x))
    ...
    def solve_sudoku(new_sudoku):
        """
              This method solves sodoku by scanning the whole 2D array
              and replacing zeros with correct numbers.
              """
        for yy in range(9):  # vertical positioning in sudoku
            for xx in range(9):  # second positioning in sudoku
